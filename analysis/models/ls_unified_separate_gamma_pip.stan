...
parameters {
  ...
  vector[3] gamma_raw; // [PCM, LNRT, ZINB]
  vector<lower=0, upper=1>[3] pip; // Posterior inclusion probability
...
model {
  ...
  for (k in 1 : 3) {
    real log_prob_spike = log(1 - pip[k]) + normal_lpdf(gamma_raw[k] | -5, 1);
    real log_prob_slab = log(pip[k]) + normal_lpdf(gamma_raw[k] | .5, 1);
    target += log_sum_exp(log_prob_spike, log_prob_slab);
  }
  vector[3] gamma = 5.0 * inv_logit(gamma_raw); // Scale gamma [0, 5]
  ...
  for (n in 1 : N) {
    ...
    // --- PCM Likelihood ---
    real effective_theta = theta[n] - gamma[1] * dist;
    eta[1] = 0;
    for (k in 2 : n_cats) {
    ...
    // --- LNRT Likelihood ---
    real mu = beta[item] - tau[person] + gamma[2] * dist;
    real sigma = 1 / alpha[item];
    log_rt[n] ~ normal(mu, sigma);
    
    // --- ZINB Likelihood ---
    real logit_pi = omega[person] - lambda[item];
    real log_mu = (kappa[person] - gamma[3] * dist) - delta[item];
...
generated quantities {
  vector[3] gamma = 5.0 * inv_logit(gamma_raw);
}
