...
// Description:
//   Stan implementation of the LS-Unified model, which unifies three psychometric models:
...
//   All components share a common latent space structure, with a single gamma parameter modulating the effect of person-item distance across all submodels.
//   The gamma parameter is assigned a spike-and-slab mixture prior using Stan's log_sum_exp formulation, with a probability of inclusion (pip) that allows for model selection between a null and substantive effect of latent space distance.
...
//   - gamma: Latent space distance multiplier (shared across all submodels, with spike-and-slab prior)
...
model {
  ...
  pip ~ beta(1, 1);
  // Mixture components for gamma_raw.
  // Prior from https://doi.org/10.3390/jintelligence12040038
  real log_prob_spike = log(1 - pip) + normal_lpdf(gamma_raw | -5, 1);
  real log_prob_slab = log(pip) + normal_lpdf(gamma_raw | .5, 1);
  target += log_sum_exp(log_prob_spike, log_prob_slab);
  real gamma = 5.0 * inv_logit(gamma_raw); // Scale gamma [0, 5]
...
generated quantities {
  real gamma = 5.0 * inv_logit(gamma_raw);
}
